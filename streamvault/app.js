...
//  StreamVault — app.js
//
//  API base routes (from your app.js):
//    /api/v1/users        user routes
//    /api/v1/video        video routes
//    /api/v1/tweet        tweet routes
//    /api/v1/subscriber   subscription routes
//    /api/v1/playlist     playlist routes
//    /api/v1/like         like routes
//    /api/v1/comment      comment routes
//    /api/v1/dashboard    dashboard routes
//
//  Subscription routes (subscription.routes.js):
//    POST   /subscriber/:channelId          → toggleSubscription
//    GET    /subscriber/subscribers/:id     → getUserChannelSubscribers
//    GET    /subscriber/subscribed          → getSubscribedChannels
// ═══════════════════════════════════════════════════════
  ...
  try {
    ...
    main.innerHTML = `
      <div class="profile-hdr fade">
        <div class="profile-cover-ph"></div>
        <div class="profile-av-row">
          <div class="profile-av">${initials(S.user?.fullName)}</div>
          <div><div class="profile-name">${esc(S.user?.fullName||'My Channel')}</div><div class="profile-un">@${esc(S.user?.username||'')}</div></div>
          <button class="btn accent" onclick="openEditProfile()">✏️ Edit Profile</button>
        </div>
      </div>
      <div class="empty"><div class="empty-icon">⚠️</div><div class="empty-title">Could not load channel data</div><div class="empty-sub">${esc(e.message||'')}</div></div>`;
  }
}
